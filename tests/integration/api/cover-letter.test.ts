...
import { GET } from "@/app/api/cover-letter/route";
...
vi.mock("@/lib/cover-letter-docx", () => ({
  generateCoverLetterDocx: vi.fn().mockResolvedValue(Buffer.from("mock docx content")),
}));
...
describe("Cover Letter API Route", () => {
  ...
  it("returns PDF by default", async () => {
    const request = new NextRequest("http://localhost:3000/api/cover-letter");
  ...
  it("returns PDF when format=pdf", async () => {
    const request = new NextRequest("http://localhost:3000/api/cover-letter?format=pdf");
    const response = await GET(request);
  ...
  it("returns DOCX when format=docx", async () => {
    const request = new NextRequest("http://localhost:3000/api/cover-letter?format=docx");
    const response = await GET(request);
  ...
  it("uses German when lang=de", async () => {
    const request = new NextRequest("http://localhost:3000/api/cover-letter?lang=de");
    const response = await GET(request);
  ...
  it("includes filename in Content-Disposition header", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toContain("Pranav_Gautam_Cover_Letter");
  });
  ...
  it("handles German DOCX format", async () => {
    const request = new NextRequest("http://localhost:3000/api/cover-letter?lang=de&format=docx");
    const response = await GET(request);
