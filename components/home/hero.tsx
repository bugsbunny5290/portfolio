  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-24 lg:px-8">
      <div className="max-w-3xl">
        ...
        <div className="mt-6 grid grid-cols-3 gap-2 sm:mt-8 sm:gap-4 sm:max-w-md">
          {highlights.map((item) => (
            <div key={item.label} className="text-center sm:text-left">
              <p className="text-xl font-bold text-foreground sm:text-3xl">{item.value}</p>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4 sm:mt-12 sm:gap-6">
          ...
          <span className="text-xs text-muted-foreground sm:text-sm">{personalInfo.location}</span>
        </div>
      </div>
    </section>
  );
